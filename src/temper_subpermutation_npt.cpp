...
#include "temper_subpermutation_npt.h"
#include <cmath>
#include <cstring>
#include "universe.h"
#include "domain.h"
...
//#define TEMPER_SUBPERMUTATION_NPT_DEBUG 1

/* --------------------------q-------------------------------------------- */

TemperSubPermutationNPT::TemperSubPermutationNPT(LAMMPS *lmp) : Pointers(lmp) {}

/* ---------------------------------------------------------------------- */

TemperSubPermutationNPT::~TemperSubPermutationNPT()
{
  MPI_Comm_free(&roots);
...
void TemperSubPermutationNPT::command(int narg, char **arg)
{
  ...
  if (domain->box_exist == 0)
    error->all(FLERR,"temper/subpermutation/npt command before simulation box is defined");
  if (narg != 7 && narg != 8)
    error->universe_all(FLERR,"Illegal temper/subpermutation/npt command");
  ...
  if (nevery <= 0)
    error->universe_all(FLERR,"Invalid frequency in temper/subpermutation/npt command");
  nswaps = nsteps/nevery;
  if (nswaps*nevery != nsteps)
    error->universe_all(FLERR,"Non integer # of swaps in temper/subpermutation/npt command");
  ...
  // setup tempering/subpermutation/npt runs
...
#ifdef TEMPER_SUBPERMUTATION_NPT_DEBUG
...
void TemperSubPermutationNPT::scale_velocities(int t_partner, int t_me)
{
...
void TemperSubPermutationNPT::print_status()
{
  if (universe->uscreen) {
...
int TemperSubPermutationNPT::calc_nsubpermutations(const int nelems)
{
    int fibo_0 = 0;
    int fibo_1 = 1;
    int fibo_n = 1;
...
bool TemperSubPermutationNPT::check_range
...
void TemperSubPermutationNPT::swap_elements(const int ielem, const int jelem,
...
void TemperSubPermutationNPT::init_subpermutations(const int nelems,
int **permutations)
{
  // initialization
...
int TemperSubPermutationNPT::next_state(const int istate, const int nstate)
{
  return (istate + 1) < nstate ? istate + 1 : 0;
}

void TemperSubPermutationNPT::suwa_todo_algorithm
