...
CommandStyle(temper/subpermutation,TemperSubPermutation)

#else

#ifndef LMP_TEMPER_SUBPERMUTATION_H
#define LMP_TEMPER_SUBPERMUTATION_H

#include "pointers.h"

namespace LAMMPS_NS {

class TemperSubPermutation : protected Pointers {
 public:
  TemperSubPermutation(class LAMMPS *);
  ~TemperSubPermutation();
 ...
 private:
  ...
  // function for replica sub-permutation
...
E: temper/subpermutation command before simulation box is defined

The temper/subpermutation command cannot be used before a read_data, read_restart,
...
E: Invalid frequency in temper/subpermutation command

Nevery must be > 0.

E: Non integer # of swaps in temper/subpermutation command
