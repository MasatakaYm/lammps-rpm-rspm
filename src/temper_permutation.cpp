...
// #define TEMPER_PERMUTATION_DEBUG 1

/* --------------------------q-------------------------------------------- */

TemperPermutation::TemperPermutation(LAMMPS *lmp) : Pointers(lmp) {}

/* ---------------------------------------------------------------------- */

TemperPermutation::~TemperPermutation()
{
  MPI_Comm_free(&roots);
...
void TemperPermutation::command(int narg, char **arg)
{
  ...
  if (domain->box_exist == 0)
    error->all(FLERR,"temper/permutation command before simulation box is defined");
  if (narg != 6 && narg != 7)
    error->universe_all(FLERR,"Illegal temper/permutation command");
  ...
  if (nevery <= 0)
    error->universe_all(FLERR,"Invalid frequency in temper/permutation command");
  nswaps = nsteps/nevery;
  if (nswaps*nevery != nsteps)
    error->universe_all(FLERR,"Non integer # of swaps in temper/permutation command");
  ...
  // setup tempering/permutation runs
...
#ifdef TEMPER_PERMUTATION_DEBUG
...
void TemperPermutation::scale_velocities(int t_partner, int t_me)
{
...
void TemperPermutation::print_status()
{
  if (universe->uscreen) {
...
int TemperPermutation::calc_npermutations(const int nelems)
{
  int nfactorial = 1;
...
void TemperPermutation::swap_elements(const int ielem, const int jelem,
...
void TemperPermutation::init_permutations(const int nelems, int **permutations)
{
  // initialization
...
int TemperPermutation::next_state(const int istate, const int nstate)
{
  return (istate + 1) < nstate ? istate + 1 : 0;
}

void TemperPermutation::suwa_todo_algorithm
