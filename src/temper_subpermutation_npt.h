...
CommandStyle(temper/subpermutation/npt,TemperSubPermutationNPT)

#else

#ifndef LMP_TEMPER_SUBPERMUTATION_NPT_H
#define LMP_TEMPER_SUBPERMUTATION_NPT_H

#include "pointers.h"

namespace LAMMPS_NS {

class TemperSubPermutationNPT : protected Pointers {
 public:
  TemperSubPermutationNPT(class LAMMPS *);
  ~TemperSubPermutationNPT();
...
E: temper/subpermutation/npt command before simulation box is defined

The temper/subpermutation/npt command cannot be used before a read_data, read_restart,
...
E: Invalid frequency in temper/subpermutation/npt command

Nevery must be > 0.

E: Non integer # of swaps in temper/subpermutation/npt command
