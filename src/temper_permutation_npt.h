...
CommandStyle(temper/permutation/npt,TemperPermutationNPT)

#else

#ifndef LMP_TEMPERPERMUTATION_NPT_H
#define LMP_TEMPERPERMUTATION_NPT_H

#include "pointers.h"

namespace LAMMPS_NS {

class TemperPermutationNPT : protected Pointers {
 public:
  TemperPermutationNPT(class LAMMPS *);
  ~TemperPermutationNPT();
...
E: temper/permutation/npt command before simulation box is defined

The temper/permutation/npt command cannot be used before a read_data, read_restart,
...
E: Invalid frequency in temper/permutation/npt command

Nevery must be > 0.

E: Non integer # of swaps in temper/permutation/npt command
